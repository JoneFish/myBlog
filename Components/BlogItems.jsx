...
const BlogItems = ({ title, description, category, image, id }) => {
  ...
  return (
    <div className='rounded-xl overflow-hidden max-w-[330px] sm:max-w-[300px] bg-white border border-gray hover:shadow-[-7px_7px_0px_#efefef] hover:translate-y-[-2px] transition duration-300 ease-in-out'>
      <Link href={`/blogs/${id}`}>
        {/* Image section */}
      ...
      <div className="p-5">
        ...
        查看详情 <Image src={assets.arrow} alt='' width={12} className='ml-2' />
        </Link>
      </div>
    </div>
  );
